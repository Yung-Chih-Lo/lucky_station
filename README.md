...
透過「坐火行」，您可以擺脫選擇的煩惱，讓緣分帶您前往下一站的未知風景——抽完還能留下旅行心得，和其他旅人分享你的故事。
...
-   **🎯 隨機抽取車站 (Random Station Picker):**
    根據使用者勾選的縣市範圍，隨機選擇一個台鐵車站作為目的地。每次抽站都會記錄至資料庫，供排行榜統計使用。
...
-   **✍️ 匿名旅行留言 (Anonymous Travel Comments):**
    抽站後會產生一組專屬的一次性連結（token），使用者可透過該連結匿名留下 10～500 字的旅行心得。每個連結只能使用一次，防止重複留言。
-   **🏆 抽站排行榜 (Station Pick Rankings):**
    `/stats` 頁面顯示所有車站被抽中的次數排行，前三名以金銀銅牌標示，並顯示累計抽站總次數。
-   **💬 旅人心得 Feed (Traveler Stories Feed):**
    `/explore` 頁面集結所有旅人的留言心得，可依車站名稱篩選或關鍵字搜尋，分頁瀏覽所有故事。
...
| `/stats` | 抽站排行榜 |
| `/explore` | 旅人心得 Feed |
| `/comment?token=xxx` | 匿名留言表單（需持有抽站後取得的 token） |
...
3.  **啟動開發伺服器 (Start the development server):**
    ...
    npm run dev
    ```
4.  啟動成功後，在瀏覽器中打開 `http://localhost:3000`。
...
7.  抽站完成後，您會取得一組專屬連結，點擊後可到留言頁面**匿名分享你的旅行心得**（每個連結限用一次）。
8.  想看看大家都抽到哪些車站？前往「**抽站排行榜**」查看即時排名，或到「**旅人心得**」閱讀其他旅人的故事。
